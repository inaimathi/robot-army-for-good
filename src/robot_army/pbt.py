...
from . import pbtagent, prepare

logger = getLogger("robot_army.pbt")

AGENT_NAME = "pbt_agent_030"
...
def run_pbt_test(path: str):
    ...
    def _per_file_stream(f: str):
        ...
        prompt = Template(util.slurp("resources/pbt_file_prompt.md")).safe_substitute(
            repo_path=path,
            files=src_files,
            file_path=f,
        ...
        try:
            ...
            if file_report:
                shrink_prompt = Template(
                    util.slurp("resources/pbt_shrinker_prompt.md")
                ).safe_substitute(
                    repo_path=path,
                    files=src_files,
                    file_path=f,
    ...
    per_file = BiStream.ensure(likely_src).mapcat(_per_file_stream)
    ...
    full = per_file.tap(
        ...
        lambda: LLM.stream_checked(
            ...
            util.slurp("resources/pbt_reporter_prompt.md"),
